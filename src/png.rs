...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        let pos = self
            ...
            .ok_or("remove error: chunk not found")?;
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut b = Vec::new();
        b.extend(Self::STANDARD_HEADER);
        b.extend(self.chunks.iter().flat_map(|c| c.as_bytes()));
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(mut value: &[u8]) -> Result<Self> {
        ...
        while !value.is_empty() {
            ...
            let length = (chunk.length() + 4 + 4 + 4) as usize;
            chunks.push(chunk);
            value = &value[length..];
        }
